...
def _detect_hand_landmarks(
    image, flip=False, min_detection_confidence=0.8, min_tracking_confidence=0.5
):
...
def detect_hand_landmarks(
    ...
    if (
        ...
        or validate_file(source, ["vid"], raise_exception=False)
    ):
        # source is a camera index or path of a video file or an ip camera url
        results = []
        flip = False
        if type(source) == int:
            flip = True
        ...
        while True:
            ...
            if not ret:
                break
            frame_mod, result = _detect_hand_landmarks(frame, flip, min_detection_confidence, min_tracking_confidence)
    ...
    elif type(source) == np.ndarray or validate_file(
        source, ["img"], raise_exception=False
    ):
        ...
        image_mod, results = _detect_hand_landmarks(image, min_detection_confidence, min_tracking_confidence)
        if show_image:
