...
class RandomShear(tf.keras.layers.Layer):
    ...
    def _apply_random_shear(self, x, row_axis=0, col_axis=1, channel_axis=2, order=1, cval=0.0):
        # Input sanity checks:
        ...
        if self.shear != 0:
            self.shear = np.deg2rad(self.shear)
            shear_matrix = np.array(
                [[1, -np.sin(self.shear), 0], [0, np.cos(self.shear), 0], [0, 0, 1]]
            )
