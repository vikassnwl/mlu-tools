...
def from_arr(
    ...
    for i, rand_idx in enumerate(rand_indices):
        ...
        if custom_titles is not None:
            ...
            if y_preds is not None:
                cname = class_names[pred_labels[rand_idx].item()] if class_names else pred_labels[rand_idx].item()
                ...
                if pred_labels[rand_idx].item() == y[rand_idx].item():
                    ...
                    if class_names:
                        ax.text(0, true_class_y, class_names[y[rand_idx].item()], color='white', bbox=dict(facecolor='green'))
                    else:
                        ax.text(0, true_class_y, y[rand_idx].item(), color='white', bbox=dict(facecolor='green'))
...
def from_ds(
    ...
    for images, labels in image_ds:
        if i == total_items_to_show: break
